...
class DetailTahfidzPage extends StatefulWidget {
  final int id;

  const DetailTahfidzPage({required this.id, super.key});

  @override
  State<DetailTahfidzPage> createState() => _DetailTahfidzPageState();
}

class _DetailTahfidzPageState extends State<DetailTahfidzPage> {
  ...
  Future<SekolahMutabaahDetailResponse> fetchData() async {
    ...
    final res = await api.request<SekolahMutabaahDetailResponse>(
      Endpoints.mutabaahSekolahTahfidzDetail(widget.id),
      RequestType.GET,
      token: token,
