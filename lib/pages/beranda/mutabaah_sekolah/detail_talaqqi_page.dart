...
class DetailTalaqqiPage extends StatefulWidget {
  final int id;

  const DetailTalaqqiPage({required this.id, super.key});

  @override
  State<DetailTalaqqiPage> createState() => _DetailTalaqqiPageState();
}

class _DetailTalaqqiPageState extends State<DetailTalaqqiPage> {
  ...
  Future<SekolahMutabaahDetailResponse> fetchData() async {
    ...
    final res = await api.request<SekolahMutabaahDetailResponse>(
      Endpoints.mutabaahSekolahTalaqqiDetail(widget.id),
      RequestType.GET,
      token: token,
